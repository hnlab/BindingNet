...
def prep_folders(out_dir, chembl_ids):
  ...
  chembl_dirs = [
      out_dir / f'web_client_{chembl_id}' for chembl_id in chembl_ids
  ]

  for chembl_dir in chembl_dirs:
    if not Path.exists(chembl_dir):
      Path.mkdir(chembl_dir)

  query_files = [
      chembl_dir / f'{chembl_dir.name}-activity.tsv'
      for chembl_dir in chembl_dirs
  ]
  smiles_files = [
      chembl_dir / f'{chembl_dir.name}-smiles-chembl_id.smi'
      for chembl_dir in chembl_dirs
  ]
...
WRKDIR = Path('/home/xli/git/BindingNet')

log_file = WRKDIR/f'3-query_ChEMBL/query_v2019.{time.strftime("%Y%m%d-%H%M%S")}.log'
logging.basicConfig(filename=log_file, filemode='w', level=logging.WARNING) 
logger = logging.getLogger(__name__)

targets2chembl = extract_targets(
  WRKDIR/f'3-query_ChEMBL/converted_PDBIDs_INDEX_general_PL_data.2019.tab.tsv')
chembl_ids = list(targets2chembl.keys())
logger.warning(
  f'[{time.ctime()}] Imported {len(chembl_ids)} chembl targets.')

out_dir = WRKDIR / 'v2019_dataset'
