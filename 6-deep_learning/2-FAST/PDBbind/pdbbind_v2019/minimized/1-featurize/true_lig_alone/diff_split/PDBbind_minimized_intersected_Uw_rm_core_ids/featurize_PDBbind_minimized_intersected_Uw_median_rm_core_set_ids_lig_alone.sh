...
python 1-featurize/featurize_PDBbind_minimized_true_lig_alone_diff_split.py \
    ...
    --output dataset/diff_split/PDBbind_minimized_intersected_Uw_rm_core_ids/lig_alone/
    # --rec true_rec
printf "%s End on %s.\n" "[$(date +"%Y-%m-%d %H:%M:%S")]" $(hostname)
# &> log/PDBbind_minimized_intersected_Uw_median_rm_core_set_ids_lig_alone.log
