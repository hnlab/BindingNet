...
parser.add_argument("--input", default="/pubhome/xli02/project/PLIM/v2019_dataset")
...
def write_hdf(input_df, set_name, failure_dict, element_dict, process_type, dataset_name):
    ...
    # pdbbind_original_dir = '/pubhome/xli02/Downloads/dataset/PDBbind/PDBbind_v2019/general_structure_only'
    pdbbind_minimized_dir = '/pubhome/xli02/project/PLIM/v2019_dataset/PDBbind_v2019'
    with h5py.File(f'{args.output}/{set_name}_{args.rec}.hdf', 'w') as f:
        for _, row in tqdm(input_df.iterrows(), total=input_df.shape[0]):
            name = row['unique_identify']
            ...
            if '_' in name:
                ...
                # candi_lig = f'{pdbbind_original_dir}/{name}/{name}_ligand.mol2'
                # rec_mol2 = f'{pdbbind_original_dir}/{name}/{name}_pocket_6A.mol2'
                candi_lig = f'{pdbbind_minimized_dir}/{name}/cry_lig_opt_converted.sdf'
...
def main():
    ...
    if args.metadata:
        ...
        valid_name = f'{args.dataset_name}_valid'
