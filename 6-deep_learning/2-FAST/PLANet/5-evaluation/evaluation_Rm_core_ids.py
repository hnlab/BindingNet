...
# test_types = ['valid', 'train', 'test', 'CASF_v16_minimized', 'CASF_v16_intersected_Uw_minimized'] #change by the order in log
...
for log_dir_name in log_dir_names:
    for mdl_type in mdl_types:
        if log_dir_name in ['PDBbind_minimized', 'PDBbind_minimized_intersected_Uw']:
            log_dir =  f'{wrkdir}/PDBbind/pdbbind_v2019/minimized/2-train/true_lig_alone/test/rm_core_ids/{log_dir_name}/{mdl_type}/log'
        else:
            log_dir = f'{wrkdir}/3-test/scripts/true_lig_alone_modify_dists/{mdl_type}/rm_core_ids/{log_dir_name}/log/'
...
sum_df.to_csv(f'{wrkdir}/5-evaluation/PDBbind_vs_PLANet_Rm_core_set/PDBbind_vs_PLANet_Rm_core_set.csv', sep='\t', index=False)
...
grouped_median.to_csv(f'{wrkdir}/5-evaluation/PDBbind_vs_PLANet_Rm_core_set/PDBbind_vs_PLANet_Rm_core_set_median.csv', sep='\t', index=False)

grouped = sum_df.groupby('test_type')
for metric in ['R2', 'mae', 'mse', 'pearsonr', 'spearmanr']:
    out_dir = f'{wrkdir}/5-evaluation/PDBbind_vs_PLANet_Rm_core_set/{metric}'
...
for metric in ['R2', 'mae', 'mse', 'pearsonr', 'spearmanr']:
    out_dir = f'{wrkdir}/5-evaluation/PDBbind_vs_PLANet_Rm_core_set/{metric}/performance_inner_dataset'
    if not Path(out_dir).exists():
        Path(out_dir).mkdir()
    for test_data in log_dir_names:
        data_df = data_grouped.get_group(test_data)
        fig, ax= plt.subplots(figsize=(8,6))
        sns.boxplot(x="test_type", y=metric, data=data_df, hue="model_type", order = ['train', 'valid', 'test', 'CASF_v16_minimized', 'CASF_v16_intersected_Uw_minimized', 'CASF_v16_original', 'CASF_v16_intersected_Uw_original'], linewidth=2.5)
        sns.swarmplot(x="test_type", y=metric, data=data_df, hue="model_type", order = ['train', 'valid', 'test', 'CASF_v16_minimized', 'CASF_v16_intersected_Uw_minimized', 'CASF_v16_original', 'CASF_v16_intersected_Uw_original'], size = 4, dodge=True, edgecolor="black", linewidth=0.7)
        ax.set_title(f'{metric} on {test_data} set')
        fig.autofmt_xdate()
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles[:2], labels[:2])
        vertical_offset = data_df[metric].median() * 0.07 # offset from median for display
        for i, test_tp in enumerate(['train', 'valid', 'test', 'CASF_v16_minimized', 'CASF_v16_intersected_Uw_minimized', 'CASF_v16_original', 'CASF_v16_intersected_Uw_original']):
            for tp in ['cmx', 'lig_alone']:
